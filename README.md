...
1. При помощи команды docker images проверить id или название (REPOSITORY) того образа, что мы хотим загрузить. Если при создании образа мы не давали ему имя - то тогда юзам его id
3. При помощи команды docker tag - переименовуем образ чтобы его имя включала сначало ник юзара а затем уже имя образа: <User_DockerHub_Name>/<IMAGE_ID_or_Name>
2. Используя команду 'doker push <User_DockerHub_Name>/<IMAGE_ID_or_Name>' заливаем его в DockerHub