...
# И после настройки параметров нашего образа в консоли запускаем: docker build <APP_PATH> - чтобы запустить билд приложения
...
# Но в чем же разница между 'docker run <IMAGE_ID>' vs 'docker start <CONTAINER_ID>'?
...
# Чтобы удалить какой то конкретный контейнер (один или больше) - docker rm <CONTAINER_ID_or_Name> <CONTAINER_ID_or_Name>
# Чтобы удалить все контейнеры за раз (как активные так и нет, наверное) - docker container prune
...
# Для этого в 'docker run <IMAGE_ID>' нужно добавить еще пару параметров. Первый '-p' (порт) и после него указываем какой именно набор портов, например '3000:3000, 80:3000 и т.д.'
...
# То есть 'порт_который_хотим_использовать_локально : порт_внутри_контейнера (e.g. EXPOSE 3000)'
...
# Однако каждый раз когда мы запускаем docker run -p 3000:3000 <IMAGE_ID>, мы можем заметить, что мы погружаемся в текущею консоль и больше ничего в ней не можем писать пока не завершим процесс
