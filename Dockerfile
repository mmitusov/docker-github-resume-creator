...
# Если мы предваритеьлно не предустанавливали image, то докер автоматом попытается найти его у себя
# Говорим что наш образ мы будем базировать на основе Реакта
FROM node 
...
# Либо же мы можем заранее задать второй параметр при помощи команды WORKDIR (рабочая директория приложения)
# И уже в WORKDIR укажем местоположение где лежат все наши файлы
# И если мы используем WORKDIR, то в COPY вторым параметром можно просто оставить точку
COPY . .
...
# P.S. Команда RUN запускается каждый раз когда мы строим наш образ, а CMD когда мы уже запускаем наш готовый образ

# Команда которая говорит на каком порте будет запущенно наше приложение. Команда не обязательна но считается как best practice
EXPOSE 3000

# И после настройки параметров нашего образа в консоли запускаем: docker build <APP_CLI_PATH> - чтобы запустить билд приложения
# После билда мы можем посмотреть список наших готовых образов при помощи команды: docker images. В списке мы увидем как имя нашего билда так и его уникальный IMAGE ID
...
# Чтобы остановить любой из запущенных контейнеров, воспользуемся командой: docker stop <CONTAINER_ID>
# После чего мы увидем что наш контейнер пропал со списка запущенных ('docker ps')
# Но при он будет отображаться просто в списке всех контейнеров (не запущенных). Отобразить список всех контейнеров можно при помощи команды: docker ps -a
# И при необходимости мы можем заново запусакть контейнеры из этого списка, при помощи конанды docker start <CONTAINER_ID>

# Каждый раз когда мы запускаем команду docker run <IMAGE_ID> - мы всегда создаем НОВЫЙ контейнер на основе нашего image
...
# Для этого в 'docker run <IMAGE_ID>' нужно добавить еще пару параметров. Первый '-p' (порт) и после него указываем какой именно набор портов, например '3000:3000'
# docker run -p 3000:3000 <IMAGE_ID>
# Первый порт говорит нам какой локальный порт использовать на нашей машине, чтобы запустить контейнер. А второй - кокой порт из контейнера мы хотим замапить на наш локальный
# То есть 'порт_который_хотим_использовать_локально : порт_внутри_контейнера'
